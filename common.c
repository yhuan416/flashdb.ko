...
char param_part_name[FAL_DEV_NAME_MAX] = "kvdb";
...
int is_valid_device(const char *name)
{
    return !strcmp(param_mtd_name, name);
}

int is_valid_partition(const char *name)
{
    return !strcmp(param_part_name, name);
}
